...
def main() -> None:
    ...
    output_path = OUTPUT_DIR / "polygon_or_showcase.yaml"
    ...
    result_segments = cg.polygon_or(layer1_segments, layer2_segments)
    ...
    document.add_layer("polygon_or_segments", segments=result_segments)
    document.add_layer("polygon_or_polygons", polygons=result_polygons)
    ...
    print(f"Union segments: {len(result_segments)}")
    print(f"Union polygons: {len(result_polygons)}")
    print(f"Union area: {sum(polygon.area() for polygon in result_polygons)}")
    print(f"Wrote {output_path}")

    plot = cg.Plot("polygon or showcase", "x", "y")
    for polygon in result_polygons:
        plot.add_polygon(polygon, face_color="purple", edge_color="indigo", alpha=0.35)
