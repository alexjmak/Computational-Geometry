...
def main() -> None:
    ...
    output_path = OUTPUT_DIR / "polygon_difference_showcase.yaml"
    ...
    result_segments = cg.polygon_difference(layer1_segments, layer2_segments)
    ...
    document.add_layer("polygon_difference_segments", segments=result_segments)
    document.add_layer("polygon_difference_polygons", polygons=result_polygons)
    ...
    print(f"Difference segments: {len(result_segments)}")
    print(f"Difference polygons: {len(result_polygons)}")
    print(f"Difference area: {sum(polygon.area() for polygon in result_polygons)}")
    print(f"Wrote {output_path}")

    plot = cg.Plot("polygon difference showcase", "x", "y")
    for polygon in result_polygons:
        plot.add_polygon(polygon, face_color="tomato", edge_color="darkred", alpha=0.4)
